...
def home2(request):
    visitor = Student.objects.all()
    return render(request, 'visit/home2.html', {'visitor1':visitor})


def sineup(request):
    form = SineUpForm()
    if request.method=="POST":
        if request.POST['username'][0]==" " or request.POST['address'][0] ==" " or request.POST['firstname'][0] ==" " or request.POST['lastname'][0] ==" ":
             return render(request,'visit/sineup.html', {'form': form,'error3':"Name, Address, Firstname and Lastname can't be empty or Blankspace should not allowed on first position"})
...
def register(request):
    ...
    if request.method=="POST":
        ...
        form = SineUpForm(request.POST, request.FILES)
        if request.POST['username'][0]==" " or request.POST['address'][0] ==" " or request.POST['firstname'][0] ==" " or request.POST['lastname'][0] ==" ":
             return render(request,'visit/register.html', {'form': form,'error3':"Name, Address, Firstname and Lastname can't be empty or Blankspace should not allowed on first position"})
        if form.is_valid():
            form.save()
            ...
            return render(request,"visit/register.html",{'error':"User Alredy Exist with this(username/email/Contact no.) details",'form': form})
...
def newvisitor(request):
    ...
    if request.method == 'POST':
        ...
        if request.POST['name'][0]==" " or request.POST['address'][0] ==" " or request.POST['college'][0] ==" " or request.POST['purpose'][0] ==" ":
             return render(request,'visit/newvisitor.html', {'form': form,'error3':"Name, Address, Collage name and Purpose can't be empty or Blankspace should not allowed on first position"})
        if form.is_valid():
...
def home(request):
    form = StudentVisitForm()
    if request.method == 'POST':
        form = StudentVisitForm(request.POST)
        # if request.POST['name'][0]==" " or request.POST['address'][0] ==" " or request.POST['college'][0] ==" " or request.POST['purpose'][0] ==" ":
        #     return render(request,'visit/home.html', {'form': form,'error3':"Name, Address, Collage name and Purpose can't be empty or Blankspace should not allowed on first position"})
        print(request.POST)
        if form.is_valid():
            status = request.POST['status']
            from_time=request.POST['from_time']
...
def loginpage(request):
    if request.method == 'POST':
        ...
        if user is not None:
            ...
            a= messages.info(request, 'Username or Password is incorrect!!')
            return render(request, 'visit/login.html',{"error1":'Username or Password is incorrect!!'})
...
def dashboard(request):
    ...
    if request.user.is_superuser:
        ...
        visitor = Student.objects.filter(status="Accepted")|Student.objects.filter(status="Completed")
        visitor=visitor.filter(date=datetime.now().date()).order_by('date').reverse()
        data = visitor.values()
...
def pagination(visitor, page_number,entry=10):
    paginator = Paginator(visitor, entry, orphans=1)
...
def searchVisitor(request):
    ...
    if len(search_text.strip()) > 0:
        ...
        visitor=visitors.order_by('date').reverse()
        entry=int(request.GET.get('entry'))
        print(entry)
        page_number = request.GET.get('page')
        context = pagination(visitor, page_number,entry)
        context['entry']=entry
    ...
    print(entry)
    page_number = request.GET.get('page')
    visitors = Student.objects.all().order_by('date').reverse()
    context = pagination(visitors, page_number,entry)
    context['entry']=entry
    context['searchText']=search_text
    # return render(request, 'visit/visitor1.html', {"visitors": visitors})
    return render(request, 'visit/visitor1.html', context)
...
@login_required(login_url='/login/')
@permission_required('visit.is_Security')
def SecuritySearchVisitor(request):
    ...
    if len(search_text.strip()) > 0:
        visitors = Student.objects.filter(
            ...
            status__istartswith=search_text) | Student.objects.filter(
            status__istartswith=search_text)
        visitor=visitors.filter(status="Accepted").order_by('id').reverse()
        page_number = request.GET.get('page')
        entry=int(request.GET.get('entry'))
        context = pagination(visitor, page_number,entry)
        context['entry']=entry
    ...
    search_text=""
    page_number = request.GET.get('page')
    visitors = Student.objects.filter(status="Accepted").order_by('date').reverse()
...
def EditVisitorTime(request, id):
    ...
    if request.method == 'POST':
        ...
        if fm.is_valid():
            ...
            date2=date_time_obj.date()
            if from_time1<time:
                return render(request,'visit/VisitorDetail.html', {'form': form,'student': st,'error':'please enter a valid time duration time is not before now'})
...
def RemaningTimeToMeet(request,pk):
    ...
    if datetime1>datetime3:
        ...
        context["option"]="We are meet with you soon"
...
def Applicant_edit(request, id):
    bus = get_object_or_404(Applicant, pk=id)
    if request.method == "POST":
        form = SineUpForm1(request.POST, instance=bus)
        print('hello')
        # if request.POST['username'][0]==" " or request.POST['address'][0] ==" " or request.POST['firstname'][0] ==" " or request.POST['lastname'][0] ==" ":
        #      return render(request,'visit/Applicant_edit.html', {'form': form,'error3':"Name, Address, Firstname and Lastname can't be empty or Blankspace should not allowed on first position"})
        print(request.POST['id_name'])
        if form.is_valid():
            print('abc')
            ...
            return render(request, 'visit/Applicant_edit.html', {'form': form ,'error4':'not valid'})
    else:
        form = SineUpForm1(instance=bus)
        return render(request, 'visit/Applicant_edit.html', {'form': form})
